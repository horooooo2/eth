...
export type LogEventLike = {
  ...
  position_id?: string;
  reduce_only?: boolean | string | number;
  purpose?: string;
...
function detailsOf(event: LogEventLike) {
...
function pickQty(event: LogEventLike, keys: string[]) {
...
export function canonicalInstrumentId(symbol?: unknown) {
...
export function displaySymbol(symbol?: unknown) {
  return canonicalInstrumentId(symbol) || '—';
}

export function hasRealPositionId(event: LogEventLike) {
  const pid = norm(event.position_id || detailsOf(event).position_id);
  if (!pid) return false;
  return !['NONE', 'NULL', 'N/A', '0', '-'].includes(pid.toUpperCase());
}

function isReduceOnly(event: LogEventLike) {
  const details = detailsOf(event);
  const flag = event.reduce_only ?? details.reduce_only;
...
export function filledQty(event: LogEventLike) {
  return pickQty(event, QTY_KEYS);
}

export function classifyDisplayCategory(event: LogEventLike): 'POSITION' | 'SYSTEM' {
...
function reasonCodes(event: LogEventLike): string[] {
  ...
  if (!codes.length && event.reason_code) codes = [String(event.reason_code).trim()].filter(Boolean);
  return codes;
}

function qtyLine(filled: number | null, target: number | null) {
...
export function formatUserMessage(event: LogEventLike): string {
  const et = norm(event.event_type).toUpperCase();
...
export function annotateEvent<T extends LogEventLike>(event: T) {
  return {
    ...event,
    symbol_display: displaySymbol(event.symbol),
    display_category: classifyDisplayCategory(event),
    display_message: formatUserMessage(event),
  };
}

export function isPositionEvent(event: LogEventLike | string) {
  if (typeof event === 'string') {
    return classifyDisplayCategory({ event_type: event }) === CATEGORY_POSITION;
  }
  return classifyDisplayCategory(event) === CATEGORY_POSITION;
}
