...
export const STRATEGY_NAME_ZH: Record<string, string> = {
  S1: '趋势跟踪',
  S2: '极端情绪反转',
  S3: '市场状态',
  S4: '执行时机',
...
export const DIRECTION_STATE_ZH: Record<string, string> = {
...
export function directionStateZh(state?: string | null): string {
  const key = String(state || '').trim().toUpperCase();
  return DIRECTION_STATE_ZH[key] || DIRECTION_STATE_ZH.NEUTRAL;
}

const EARLY_DIR_PHRASE: Record<string, string> = {
...
export function s9ReasonZh(
  ...
  if (templates[key] && phrase) {
    const flow = state === 'EARLY_BEARISH' || String(extras?.side || '').toUpperCase() === 'SHORT' ? '卖盘' : '买盘';
...
export function strategyNameZh(strategyId?: string | null): string {
