#!/usr/bin/env bash
# Server-side deploy: Node WhaleTracker only.
...
# AI Trader sidecar is gone — stop unit and free :8000.
echo "==> retire ai-trader service (if present)"
systemctl stop ai-trader >/dev/null 2>&1 || true
systemctl disable ai-trader >/dev/null 2>&1 || true
rm -f /etc/systemd/system/ai-trader.service
systemctl daemon-reload
systemctl reset-failed ai-trader >/dev/null 2>&1 || true
if ss -lntp 2>/dev/null | grep -q ':8000'; then
  echo "WARN: something still listens on 8000 after retiring ai-trader"
  ss -lntp | grep ':8000' || true
fi
echo "==> ai-trader retired"
...
echo "==> drop retired strategy / AI Trader modules from $DEPLOY"
...
# Strategy/exchange/AI Trader env is dead config now — remove it.
echo "==> strip retired strategy/exchange/AI Trader ENV from $DEPLOY/.env"
for key in V41_ENGINE_ENABLED V41_ENGINE_BASE_URL V41_ENGINE_INTERNAL_TOKEN \
           ...
           OKX_API_KEY OKX_API_SECRET OKX_API_PASSPHRASE OKX_ALLOW_ENV_CREDS \
           AI_TRADER_URL AI_TRADER_PROXY_TIMEOUT_MS AI_TRADER_DB AI_TRADER_NARRATOR; do
...
for i in $(seq 1 20); do
  ...
  # AI Trader proxy must be gone.
  code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 5 http://127.0.0.1/ai-api/health || true)
  echo "  /ai-api/health => HTTP $code (expect 404)"
  if [ "$code" != "404" ]; then
    echo "ERROR: /ai-api still reachable (HTTP $code)"
    exit 8
  fi
  ok=1
  break
...
echo "==> deploy finished successfully (node only)"
